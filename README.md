...
flowchart TD
    ...
    SALES -->|"WHERE item_name = ITEM_NAME<br/>（既定 '00納豆' / 全8商品から選択）"| FC
...
`daily_sales` には「00納豆」以外の商品も入っています。
...
| :--- | :--- | :--- | :--- |
| `00納豆` | **予測対象の主役**。週次周期性のみ | +20% | なし（意図的に無効化） |
...
| `00納豆` / `絹ごし豆腐` | 中 | 週次周期性が明確。TimesFM が最も得意とする形 |
