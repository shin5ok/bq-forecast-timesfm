...
「来週の 20% OFF」「チラシ掲載」「エンド陳列」「アプリクーポン」「周年祭」のような
...
- [4. Layer A — 後段補正を精密化する](#4-layer-a--後段補正を精密化する)
- [5. Layer B — `ARIMA_PLUS` のカスタム休日](#5-layer-b--arima_plus-のカスタム休日)
- [6. Layer C — `ARIMA_PLUS_XREG`（本命）](#6-layer-c--arima_plus_xreg本命)
- [7. Layer D — Vertex AI Forecasting](#7-layer-d--vertex-ai-forecasting)
- [8. 番外 — TimesFM を covariate 付きで自前運用する](#8-番外--timesfm-を-covariate-付きで自前運用する)
...
1. **`AI.FORECAST` は単変量 API**で、外生変数（covariate）を渡す引数が存在しない。これは制限ではなく仕様。
2. 施策を効かせる方法は **「予測の後ろで補正する」か「モデルを `AI.FORECAST` 以外に替える」** の二択しかない。
3. BigQuery 内で完結させたいなら **`ARIMA_PLUS_XREG` が事実上の唯一解**。ゼロショットは捨てることになる。
...
> **最初の分岐がすべてです。**
> 「予測対象日ぶんの値が事前に分かるか」で使える／使えないが決まります。
> Vertex AI Forecasting はこれを **available at forecast / unavailable at forecast** という
> 用語で明示的に区別しています（→ [7 章](#7-layer-d--vertex-ai-forecasting)）。
> 販促施策は普通「計画」なので available 側 = **予測に使える**、実測気温は unavailable 側です。
...
**説明変数を渡す引数がありません**。`id_cols` は系列を分けるキーであって特徴量ではありません。
...
TimesFM は実績値の並びからパターンを**暗黙に**復元します。
したがって施策のうち、**過去に規則的に繰り返されたもの**は間接的に拾えます。

| 施策の性質 | TimesFM が実績値だけから拾えるか |
| :--- | :--- |
| 毎週水曜が定番の特売日 | **拾える**（週次周期性として現れるため） |
| 毎月 0 のつく日のポイント 5 倍 | 拾えることがある（10 日周期が十分な回数あれば） |
| 今回だけの 30% OFF | **拾えない** |
| 今回だけチラシ 1 面に載せた | **拾えない** |
| 割引率が 10% と 30% で効きが違う | **拾えない**（水準の区別を持てない） |
...
SELECT
  ...
  a.sales_qty,
  -- ここから下が施策特徴量。過去も未来も必ず値が入る（NULL を残さない）
...
これらが気になり始めたら Layer C に進む合図です。

---

## 5. Layer B — `ARIMA_PLUS` のカスタム休日

**日付だけで表せる施策**（祝日、周年祭、地域の祭事、全店一斉のセール）なら、
`ARIMA_PLUS` の休日効果モデリングがそのまま使えます。特徴量テーブルを作る必要がありません。
...
| 制約 | 内容 |
| :--- | :--- |
| **履歴 1 年超が必須** | 休日効果モデリングは **日次または週次、かつ 1 年より長い**系列にしか適用されません。条件を満たさないと `holiday_region` を指定しても**黙って無視されます**。このリポジトリの既定 `HISTORY_DAYS=180` では効きません。試すなら `make setup HISTORY_DAYS=730` |
...
## 6. Layer C — `ARIMA_PLUS_XREG`（本命）

**割引率・チラシ・陳列を「値」としてモデルに渡したい**なら、これが BigQuery 内での本命です。
`ARIMA_PLUS` に**線形の外生変数**を足した構造で、時系列成分（トレンド・季節性・休日）と
外生変数の寄与を分けて推定します。
...
OPTIONS (
  ...
  --    施策の効果を学習させたいなら必ず FALSE にする（→ 6-4）
...
予測値は次の分解式で構成されます（`forecast` 行の場合）:

```
time_series_data = trend
                 + seasonal_period_{yearly,quarterly,monthly,weekly,daily}
                 + holiday_effect
                 + (attribution_feature_1 + ... + attribution_feature_n)
```

**「なぜこの発注数なのか」を人に説明できる**のが、`AI.FORECAST` に対する最大の実務的アドバンテージです。
「うち 32 個ぶんはチラシ掲載の効果」と数字で言えます。

### 6-4. 落とし穴（これを知らないと必ず踏む）

| # | 落とし穴 | 対処 |
| :--- | :--- | :--- |
| 1 | **`CLEAN_SPIKES_AND_DIPS` が既定 `TRUE`** | 特売日の山を外れ値と判定して局所線形補間で潰します。外生変数に学習させたい山を、前処理が先に消してしまう。**`FALSE` を明示** |
...
| 4 | **未来の特徴量が無いと予測できない** | 施策予定が確定していない日は `is_promo = 0` などの既定値で埋める。埋め忘れると 3 番の挙動に落ちる |
...
## 7. Layer D — Vertex AI Forecasting

BigQuery ML の枠を出てもよいなら、Vertex AI の予測（旧 AutoML Forecasting）は
**施策の扱いが最も体系化されています**。

### 7-1. 3 つの特徴量タイプ

Vertex AI は列を次の 3 種に分類させます。**このリポジトリのデータを当てはめると:**
...
Layer A〜C でやってきた工夫が、ここでは API の第一級概念として扱われます。
...
- BigQuery から Vertex AI データセットを直接参照できるので、**データの持ち出しは不要**。
...
## 8. 番外 — TimesFM を covariate 付きで自前運用する

「ゼロショットのまま、施策も効かせたい」——これは BigQuery の `AI.FORECAST` では不可能ですが、
**TimesFM 本体には covariate 対応の API があります**。

OSS 実装（`timesfm[xreg]`）の `forecast_with_covariates()` は次を受け取ります。
...
施策マスタが整備されていない現場では、販促計画が PDF・Excel・チラシ画像で存在します。
BigQuery から Gemini を呼んで構造化できます。
...
`ARIMA_PLUS` 系のモデルからこれを機械的に洗い出せます。
...
洗い出した日を施策マスタに追記していくのが、Layer C の精度を上げる一番地道で確実な作業です。
...
**全期間の MAPE が改善したかだけを見ないでください。** 施策日は日数が少ないので、
全体指標にはほとんど現れません。
...
2. **[4 章](#4-layer-a--後段補正を精密化する)の uplift 実測を回す。** モデルを変えずに、
   現行の固定ルール（特売日は上限値）より良くなるかを見る。ここで十分なら終わり。
3. **`HISTORY_DAYS` を 730 に増やして [Layer B](#5-layer-b--arima_plus-のカスタム休日) を試す。**
   祝日・季節イベントが効く商品（鍋つゆ・アイスクリーム）で差が出やすい。
4. **[Layer C](#6-layer-c--arima_plus_xreg本命) を 1 カテゴリで試す。**
   `clean_spikes_and_dips = FALSE` を忘れずに。`ML.EXPLAIN_FORECAST` で
   `attribution_discount_rate` の符号と大きさが直感と合うかを必ず確認する。
   符号が逆なら、施策マスタか欠品補正が間違っています。
5. **それでも足りなければ [Layer D](#7-layer-d--vertex-ai-forecasting)。**
